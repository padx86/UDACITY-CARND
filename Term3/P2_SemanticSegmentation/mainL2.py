...
import project_testsL2 as tests
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    kernel_reg = tf.contrib.layers.l2_regularizer(scale=0.001);
    kernel_init = tf.random_normal_initializer(stddev=.00001);

    l2loss=list()

    # Add Layer to layer7 output --> 1x1 convolution
    l7out_conv1x1 = tf.layers.conv2d(vgg_layer7_out, num_classes, 1, padding = 'same', kernel_regularizer=kernel_reg)
    l2loss.append(tf.nn.l2_loss(l7out_conv1x1))
    # Add Layer to layer4 ouput --> 1x1 convolution
    l4out_conv1x1 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding = 'same', kernel_regularizer=kernel_reg)
    l2loss.append(tf.nn.l2_loss(l4out_conv1x1))
    # Add Layer to layer3 output --> 1x1 convolution
    l3out_conv1x1 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding = 'same', kernel_regularizer=kernel_reg)
    l2loss.append(tf.nn.l2_loss(l3out_conv1x1))
    # Upsample L7out_conv1x1 and add
    l7out_up = tf.layers.conv2d_transpose(l7out_conv1x1, num_classes, 4, 2, padding = 'same', kernel_regularizer=kernel_reg) 
    l2loss.append(tf.nn.l2_loss(l7out_up))
    #Add l7 upsampling output and the l4 output after 1x1 convolution
    l4in_up = tf.add(l7out_up, l4out_conv1x1)
    
    #Upsamle new layer
    l4out_up = tf.layers.conv2d_transpose(l4in_up, num_classes, 4, 2, padding = 'same', kernel_regularizer=kernel_reg)
    l2loss.append(tf.nn.l2_loss(l4out_up))
    #Add l4 upsamling output and l3 output after 1x1 convolution
    l3in_up = tf.add(l4out_up, l3out_conv1x1)

    #Upsamle new layer
    l3out_up = tf.layers.conv2d_transpose(l3in_up, num_classes, 16, (8,8), padding = 'same', kernel_regularizer=kernel_reg)
    l2loss.append(tf.nn.l2_loss(l3out_up))
    print("DONE")
    #RTeturn last upsampled layer
    return l3out_up, l2loss

tests.test_layers(layers)


def optimize(nn_last_layer, correct_label, learning_rate, num_classes, l2loss):
    """
    ...
    """
    # TODO: Implement function
    beta = 0.01 # Use this factor for L2 regulazition loss
    print("Creating optimizer operation for training")
    logits = tf.reshape(nn_last_layer, (-1, num_classes))           #Resize logits to 2d
    ...
    regulization_loss = tf.add_n([ l2l for l2l in l2loss ])#Get loss caused by L2 of all trainable variables

    opt = tf.train.AdamOptimizer(learning_rate)         #use adam optimizer
    # train_opt = opt.minimize(cross_entropy_loss)        #tell Optimizer what to minimize - cross entropy only
    train_opt = opt.minimize(cross_entropy_loss + beta*regulization_loss)        #tell Optimizer what to minimize - cross entropy and l2 losses
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for ep in range(epochs): # Train for each epoch 
        ...
        for im, lbl in get_batches_fn(batch_size):
            batch_counter += 1
            current_loss = sess.run([train_op, cross_entropy_loss],                 # Run training operation
            ...
            print("Epoch: ", ep+1, " of ", epochs, " - batch number", batch_counter, ": Loss:", current_loss[1])
...
def run():
    ...
    with tf.Session() as sess:
        ...
        final_layer, l2loss = layers(l3, l4, l7, num_classes)
        # TODO: Train NN using the train_nn function
        logits, train_opt, cross_entropy_loss = optimize(final_layer, correct_label, learning_rate, num_classes, l2loss)

        #Init vars
        sess.run(tf.global_variables_initializer())

        #Define epochs, batch size, dropout and learning rate
        EPOCHS = 20
        BATCHSIZE = 2
        KEEPPROB = 0.5
        LEARNINGRATE = 0.0001
